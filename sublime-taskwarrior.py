...
class TaskWarriorAPI:
    def load_tasks(self):
        tasks = dict()
        bashCommand = 'task export status:pending'
        process = subprocess.Popen(bashCommand.split(' '), stdout=subprocess.PIPE)
        task_data = process.communicate()[0]
        tasks = json.loads(task_data.decode('ascii'))
        return tasks
...
class TaskwarriorViewTasksCommand (sublime_plugin.WindowCommand):
    ...
    def mod_task(self, idx):
        ...
        if idx == 4:
            ...
            with editwrapper.Edit(v) as edit:
                task_info = tw.task_view(twtask[u'uuid'])
                try:
                    task_info.decode('ascii')
                # If it's already been decoded or is user inputted text
                except AttributeError:
                    pass
                edit.insert(0, task_info)
                edit.insert(0, 'Details for task "' + twtask[u'description'] + '":' + '\n')
            v.show(v.size())
